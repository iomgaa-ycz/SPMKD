...
import random
import math
from util.utils import TensorPrepLib, FileNameInputLib
...
class SLP_RD_SLP:  # slp reader
    ...
    def __init__(self, opts, phase='train', if_0base=True,config=None):
        dsFd = opts.data_path
        self.dsFd = dsFd
        n_frm = 45  # fixed
        if 'simLab' in dsFd:
        ...
        for i in tqdm(idxs_subj_all, desc='initializing SLP'):
            ...
            self.li_bb_sq_RGB.append(
                np.array(list(map(lambda x: ut.get_bbox(x, rt_xy=1), joints_gt_RGB_t))))  # n_subj x 45 x4
            ...
            self.li_bb_sq_PM.append(
                np.array(list(map(lambda x: ut.get_bbox(x, rt_xy=1), joints_gt_PM_t))))  # only upper level is list

        cov_li=['uncover', 'cover1', 'cover2']
        for i in idxs_subj:
            for cov in cov_li:  # add pth Descriptor
...
class SLP_RD_privatedata:
    def __init__(self):
        super(SLP_RD_privatedata, self).__init__()
        path = "../PrivateData"
        self.PM_list = os.listdir(path)
        self.scale = 0.8  # the scale of train data and val data
        self.sz_PM = [240, 60]
        self.fc_depth = 50

        self.val = []
        self.train = []
        # while (len(self.val)<=num_val):
        #     file_num=random.randint(0,len(self.train)-1)
        #     self.val.append(self.train[file_num])
        #     del self.train[file_num]
        for PM_name in self.PM_list:
            PM_class = PM_name.split("_")
            if PM_class[0] in ["a"]:
                self.train.append(PM_name)
            else:
                self.val.append(PM_name)
        self.train_len = len(self.train)
        self.val_len = len(self.val)
        # if self.phase=="train":
        #     for index in tqdm(self.train):
        #         path_pm=path+"/"+index
        #         pm=np.load(path_pm,allow_pickle=True)
        #         self.train_data[index]=pm
        # elif self.phase=="test":
        #     for index in tqdm(self.val):
        #         path_pm=path+"/"+index
        #         pm=np.load(path_pm,allow_pickle=True)
        #         self.val_data[index]=pm
        # else:
        #     for index in tqdm(self.train):
        #         path_pm=path+"/"+index
        #         pm=np.load(path_pm,allow_pickle=True)
    ...
    def get_array_joints(self, idx_smpl=0, phase="train"):
        '''
        index sample function in with flattened order with given modalities. It could be raw form array or image, so we call it array.
        corresponding joints will be returned too.  depth and PM are perspective transformed. bb also returned
        :param idx_smpl: the index number base 0
        ...
        path = "../PrivateData"
        if phase == "train":
            path_pm = path + "/" + self.train[idx_smpl]
            arr = np.load(path_pm, allow_pickle=True)
            PM_class = self.train[idx_smpl].split("3")[0]
        else:
            path_pm = path + "/" + self.val[idx_smpl]
            arr = np.load(path_pm, allow_pickle=True)
            PM_class = self.val[idx_smpl].split("3")[0]
        # print("ok")

        return arr, PM_class


class SLP_RD_PosePressure:
    def __init__(self, opt):
        super(SLP_RD_PosePressure, self).__init__()
        x_map_ct = 5
        pmat_gt_idx = 0
        self.slp = 'synth'
        ...
        test_database_file_real_f, test_database_file_real_m, test_subj_mass_list_f, test_subj_mass_list_m = FileNameInputLib1.get_dana_slp(
            False)
        train_database_file_synth_f, train_database_file_synth_m = FileNameInputLib1.get_slpsynth_pressurepose(True,
                                                                                                               '')  # _nonoise')
        test_database_file_synth_f, test_database_file_synth_m = FileNameInputLib1.get_slpsynth_pressurepose(False,
                                                                                                             '')  # _nonoise')
        ...
        dat_f_real = None
        dat_m_real = None
        dat_f_synth = TensorPrepLib().load_files_to_database(train_files_f[1:2] + train_files_f[3:4],
                                                             creation_type='synth', reduce_data=False,
                                                             depth_in=True)
        dat_m_synth = TensorPrepLib().load_files_to_database(train_files_m[1:2] + train_files_m[3:4],
                                                             creation_type='synth', reduce_data=False,
                                                             depth_in=True)

        # allocate pressure images
        self.train_x = TensorPrepLib().prep_images(train_x, dat_f_real, dat_m_real, dat_f_synth, dat_m_synth,
                                                   filter_sigma=0.5, start_map_idx=pmat_gt_idx)
        print("ok")
